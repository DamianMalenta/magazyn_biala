...
describe('aliasMatcher', () => {
  ...
  it('matches aliases longest-first', () => {
    expect(matchSkuName('ser mozzarella', names)?.canonical).toBe('Ser Mozzarella')
    expect(matchSkuName('salami zwykle', names)?.canonical).toBe('Salami')
    expect(matchSkuName('pojemnik krewetek', names)?.canonical).toBe('Krewetki')
  })
})
